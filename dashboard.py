    ...
    def run(self):
        ...
        if not self.load_data():
            if self.is_local_environment():
                ...
                # Streamlit Cloud environment - prominent upload interface
                st.markdown("""
                ### ☁️ Welcome to Streamlit Cloud!
                
                Upload your data file to start viewing analytics and insights.
                """)
                ...
                with col2:
                    ...
                    uploaded_file = st.file_uploader(
                        "Choose your Appointments.csv file",
                        type=["csv"],
                        key="csv_uploader",
                    ...
                    if uploaded_file is not None:
                        ...
                        if st.button("✅ Upload and Process", type="primary", use_container_width=True):
                            try:
                                with st.spinner("Processing your file..."):
                                    ...
                                    import pandas as pd
                                    test_df = pd.read_csv("Appointments.csv")
                                    
                                    st.success(f"✅ Successfully uploaded {len(test_df)} appointments!")
                                    st.balloons()
                                    st.rerun()
                                    
                            except Exception as e:
                                st.error(f"❌ Error processing file: {e}")
                                st.info("Please make sure you're uploading a valid CSV file with appointments data.")
                        ...
                        ">
                            ...
                            <p>Or click "Browse files" above to select your Appointments.csv file</p>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with col1:
                    ...
                    2. **Find the generated file:**
                       - Look for `Appointments.csv` in your project folder
                ...
                with col2:
                    ...
                    ✅ **File format:** CSV (.csv)  
                    ✅ **File name:** Any name (preferably Appointments.csv)  
                    ✅ **Content:** WordPress appointments data  
                    ✅ **Size:** No specific limit  
                    
                    **Need help?** The file should contain columns like:
                    - Customer name, email, phone
                    - Appointment date, status
                    - Service, payment information
                    """)
                
                if st.button("🔄 Refresh Page", help="Refresh to check if data was uploaded", use_container_width=True):
                    st.rerun()
            return
